...
import * as Models from './models/marketplace.js';
...
export class Marketplace {
    ...
    async getCharges(
        ...
        const paramDefs: Array<{ name: string; in: string }> = [
            { name: 'meterId', in: 'query' },
            { name: 'eventId', in: 'query' },
     ...
     * Check if account has sufficient funds
     */
    async hasFunds(options?: AxiosRequestConfig): Promise<any> {
        const paramDefs: Array<{ name: string; in: string }> = [];
        const extracted = extractParams(null, paramDefs);
        const requirements: string[] = ['Location-Access-Only'];
    ...
    async uninstallApplication(
        ...
        const response: AxiosResponse<Models.DeleteIntegrationResponse> =
    ...
    async getInstallerDetails(
        ...
        const response: AxiosResponse<Models.GetInstallerDetailsResponseDTO> =
            await this.client.request(config);
        return response.data;
    }
}
