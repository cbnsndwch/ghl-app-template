...
import { UserType, UserTypeValue } from '../../constants/index.js';
import {
    buildUrl,
    extractParams,
    getAuthToken,
...
import * as Models from './models/oauth.js';
...
export class Oauth {
    ...
    public async refreshToken(
        ...
        if (!Object.values(UserType).includes(userType as UserType)) {
            throw new Error(
                `userType must be "${UserType.Location}" or "${UserType.Company}"`
            );
        }
    ...
    async getAccessToken(
        ...
        const processedBody = new URLSearchParams(
            requestBody as any
        ).toString();
        ...
        const response: AxiosResponse<Models.GetAccessCodeSuccessfulResponseDto> =
    ...
    async getLocationAccessToken(
        ...
        const response: AxiosResponse<Models.GetLocationAccessTokenSuccessfulResponseDto> =
            await this.client.request(config);
        return response.data;
    }
    ...
    async getInstalledLocation(
        ...
        const paramDefs: Array<{ name: string; in: string }> = [
            ...
            { name: 'limit', in: 'query' },
            { name: 'query', in: 'query' },
            { name: 'isInstalled', in: 'query' },
            { name: 'companyId', in: 'query' },
            { name: 'appId', in: 'query' },
            { name: 'versionId', in: 'query' },
            { name: 'onTrial', in: 'query' },
            { name: 'planId', in: 'query' }
        ];
        ...
        const response: AxiosResponse<Models.GetInstalledLocationsSuccessfulResponseDto> =
            await this.client.request(config);
        return response.data;
    }
}
